<script type="text/javascript">
		<!--
       function win1(txte) {
	        txte1 = "http://plugins.econt.com/map_locator/index/F4F4F4/333333/0297D9/FFFFFF/1/900/450/1/999999/1/0297D9/FFFFFF/F4F4F4/FFFFFF/333333";
	        txte_ob = txte1+txte;
	         window.open(txte_ob,"Window1","menubar=no,width=910,height=470,toolbar=no");
                        }
       //-->
		</script>
...
<?php
   if(MODULE_SHIPPING_ECONT_STATUS == 'True') {?>
     <table style="border: thin solid #0066CC;" width="100%" cellspacing="1" cellpadding="2" class="infoBox">
			<tr class="infoBoxContents">
				<td class="main">
				<?php echo '<h1 id="checkoutShippingHeading">' . MODULE_SHIPPING_ECONT_CHECKOUTSHIPPING_SERVICES  . '</h1>'; ?>
				</td>
			</tr>
			<tr class="infoBoxContents">
				<td class="main">
				<?php
				$checkpoint_map = $db->Execute("SELECT entry_street_address, entry_city FROM ".TABLE_ADDRESS_BOOK." WHERE address_book_id = ".(int)$_SESSION['sendto']."");
				...
				$map_location = '/bg/'. $addressbook_street .'/'. $addressbook_city;
				
				?>
				<?php 
				
				echo "<a href=\"javascript:win1('".$map_location."')\" onmouseover=\"self.status='Open A Window'; return true;\"><img  src=\"images/infobox/nearest_office_inactive.png\"      alt=\"\" onmouseover=\"this.src='images/infobox/nearest_office_active.png'\" onmouseout=\"this.src='images/infobox/nearest_office_inactive.png'\" width=\"138\" height=\"43\" border=\"0\" /></a>";
			...
			<tr class="infoBoxContents">
				<td class="main">
				<?php
				if(isset($_SESSION['pay_after_accept']) and ($_SESSION['pay_after_accept'] == 1)){echo '<span style="color: #CC3333;">'.MODULE_SHIPPING_ECONT_CHECKOUTSHIPPING_PAY_AFTER_ACCEPT_TEXT_1.'</span>';}
				if(isset($_SESSION['pay_after_accept']) and ($_SESSION['pay_after_accept'] == 2)){echo '<span style="color: #CC3333;">'.MODULE_SHIPPING_ECONT_CHECKOUTSHIPPING_PAY_AFTER_ACCEPT_TEXT_2.'</span>';}
				?>
				</td>
			</tr>
			<tr class="infoBoxContents">
				<td class="main">
				<?php
				if(isset($_SESSION['invoice_before_pay_CD']) and ($_SESSION['invoice_before_pay_CD'] == 1)){echo '<span style="color: #CC3333;">'.MODULE_SHIPPING_ECONT_CHECKOUTSHIPPING_INVOICE_BEFORE_PAY_CD_TEXT_1.'</span>';}
				?>
				</td>
			</tr>
			<tr class="infoBoxContents">
				<td class="main">
				<?php
				if(isset($_SESSION['instruction_returns']) and ($_SESSION['instruction_returns'] == 1)){echo '<span style="color: #CC3333;">'.MODULE_SHIPPING_ECONT_CHECKOUTSHIPPING_INSTRUCTION_RETURNS_TEXT_1.'</span>';}
				if(isset($_SESSION['instruction_returns']) and ($_SESSION['instruction_returns'] == 2)){echo '<span style="color: #CC3333;">'.MODULE_SHIPPING_ECONT_CHECKOUTSHIPPING_INSTRUCTION_RETURNS_TEXT_2.'</span>';}
				?>
				</td>
			</tr>
		</table>
