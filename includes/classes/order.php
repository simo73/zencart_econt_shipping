...
*/
  ...
  function create($zf_ot_modules, $zf_mode = 2) {
    ...
    if($_SESSION['payment'] == 'ecd') {$this->info['total'] = $this->info['total'] + (float)$_SESSION['econtloadingcd_user'];}
...
if($_SESSION['payment'] == 'ecd') {
	
	      $sql_data_array = array('orders_id' => $insert_id,
                              'title' => MODULE_PAYMENT_ECD_TEXT_TITLE,
                              'text' => $_SESSION['econtloadingcd_user'],
                              'value' => (float)$_SESSION['econtloadingcd_user'],
                              'class' => "ot_tax",
                              'sort_order' => "300");
...
/* END: ADD MY CUSTOM DETAILS */
  ...
  function send_order_email($zf_insert_id, $zf_mode,$zs_total) {
    ...
    if (is_object($GLOBALS[$_SESSION['payment']])) {
	  ...
	  if($payment_class == 'ecd'){
	  $email_order .= MODULE_PAYMENT_ECD_TEXT_VALUE .' : '.$_SESSION['econtloadingcd_user']. "\n\n";}
    } else {
      $email_order .= EMAIL_TEXT_PAYMENT_METHOD . "\n" .
      EMAIL_SEPARATOR . "\n";
	...
	if(EMAIL_TEXT_PAYMENT_METHOD == MODULE_PAYMENT_ECD_TEXT_TITLE){$html_msg['PAYMENT_METHOD_DETAIL']='Total:'.$_SESSION['econtloadingcd_user'];}
	else{
	...
	if($payment_class == 'ecd'){
	$mos=(float)$zs_total+(float)$_SESSION['econtloadingcd_user'];
					if($_SESSION['languages_code'] == 'en'){$moba = 'lv.';}
				else {
					if($_SESSION['languages_code'] == 'bg'){$moba = 'лв.';}
				}
	$morz = $mos . $moba;	
	$email_order .=EMAIL_SEPARATOR . "\n";
	$email_order .= 'Общо '.' : '.$morz;
	$email_order .= "\n";
	$email_order .= EMAIL_SEPARATOR . "\n";		}
    // include disclaimer
