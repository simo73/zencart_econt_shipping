	...
	switch($action) {
		...
		case 'request-courier-single':
			...
			if($blank_no->RecordCount()) {
				//$blank_no = zen_db_fetch_array($check_query);
				...
				$aLoadingInfo['sender'] = array(
					...
					'credit_num'    => $sender->fields['vendor_credit_num']
				);
				...
				#payment side
				if($sender->fields['vendor_share_sum'] == 0){$aLoadingInfo['payment']['side'] 	= 'SENDER';}
				else{
				$aLoadingInfo['payment']['side'] 	= 'RECEIVER';
				$aLoadingInfo['payment']['receiver_share_sum'] = $order_total_share_sum;//$sender->fields['vendor_share_sum'];
				
				}
				$aLoadingInfo['payment']['method'] = $sender->fields['vendor_payment_method'];
				if($sender['vendor_payment_method'] != 'CASH'){
					$aLoadingInfo['payment']['key_word'] = $aLoadingInfo['sender']['credit_num'];
				}
				...
				if($service_priority->fields['econt_cd'] == '1') {
					$aLoadingInfo['services']['cd'] = (float)$order_total;//$ot_subtotal;
				...
				if($oResponse->result->e->error != "") {
					$cityid = $db->Execute("SELECT id, post_code, name, name_en FROM ".MODULE_SHIPPING_ECONT_DB_CITIES_TABLE."  WHERE name = '".$receiver['delivery_city']."'");
					if($cityid->RecordCount()) {
						//$cityid = zen_db_fetch_array($city_query);
				...
				foreach($oResponse->result->e as $result) {
					if($result->error == "") {
						...
						$main_cd = (float)$result->loading_price->CD;
