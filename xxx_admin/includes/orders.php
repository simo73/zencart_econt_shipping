...
if(MODULE_SHIPPING_ECONT_STATUS == 'True') {
	...
	include_once(DIR_WS_INCLUDES .'modules/shipping/econt/admin/orders_econt_apps_top.php');
	} 
?>
...
<body onLoad="init()">
<!-- Econt Shipping Module BOF //-->
<?php if(MODULE_SHIPPING_ECONT_STATUS == 'True') { ?>
	<?php include_once(DIR_WS_INCLUDES .'modules/shipping/econt/admin/orders_popup_shop_config.php'); ?>
<?php } ?>
<!-- Econt Shipping Module EOF //-->
...
?>
      ...
      <!-- Econt Shipping Module BOF //-->
		<?php 
		if(MODULE_SHIPPING_ECONT_STATUS == 'True') {				
			$statuscheck = $db->Execute("SELECT * FROM ".MODULE_SHIPPING_ECONT_DB_LOADINGS." WHERE order_id = ".(int)$_GET['oID']."");
			if($statuscheck->RecordCount()) {
				//$statuscheck = tep_db_fetch_array($statuscheck_query);
				if(($statuscheck->fields['is_imported'] == '1') && ($statuscheck->fields['receiver_time'] == "0000-00-00 00:00:00")) {
					$statuscheck_icon = 'forward.png';
				} else if(($statuscheck->fields['is_imported'] == '1') && ($statuscheck->fields['receiver_time'] != "0000-00-00 00:00:00")) {
					$statuscheck_icon = 'button_ok.png';
				} else {
					$statuscheck_icon = 'next.png';
				}
				$econt_button = '<div id="loadingurl-form">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/'.$icon_next, MODULE_SHIPPING_ECONT_ICON_REQUEST_COURIER_TEXT).'</div>&nbsp;';
			} else {
				$econt_button = '<div id="requestcourier-form">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/shop_users.png', MODULE_SHIPPING_ECONT_ICON_REQUEST_COURIER_TEXT).'</div>&nbsp;'.zen_draw_hidden_field('econtorderid', $_GET['oID'], 'id="econtorderid"');
			}
			?>
			<tr>
				<td colspan="2" align="right"><?php echo $econt_button; ?><?php echo '<a href="' . zen_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $HTTP_GET_VARS['oID']) . '" TARGET="_blank">' . zen_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a> <a href="' . zen_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $HTTP_GET_VARS['oID']) . '" TARGET="_blank">' . zen_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a> <a href="' . zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('action'))) . '">' . zen_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
			</tr>
			<?php
		} else {
			?>
...
<?php if(MODULE_SHIPPING_ECONT_STATUS == 'True') { ?>
	<?php
	$statuscheck = $db->Execute("SELECT * FROM ".MODULE_SHIPPING_ECONT_DB_LOADINGS." WHERE order_id = ".(int)$orders['orders_id']."");
	if(!$statuscheck->RecordCount()) {
		$statuscheck_icon = 'shop_users.png';
	} else {
		//$statuscheck = tep_db_fetch_array($statuscheck_query);
		if(($statuscheck->fields['is_imported'] == '1') && ($statuscheck->fields['receiver_time'] == "0000-00-00 00:00:00")) {
			$statuscheck_icon = 'forward.png';
	...
	$econt_courier_button = '<div id="requestcourier-form-non">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/'.$statuscheck_icon, MODULE_SHIPPING_ECONT_ICON_REQUEST_COURIER_TEXT).'</div>';
	?>
	<td class="dataTableContent"><?php echo $econt_courier_button; ?><?php echo '<a href="' . zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders['orders_id'] . '&action=edit') . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $orders['customers_name']; ?></td>
<?php } else { ?>
	<td class="dataTableContent"><?php echo '<a href="' . zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders['orders_id'] . '&action=edit') . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $orders['customers_name']; ?></td>
<?php } ?>
<!-- Econt Shipping Module EOF //-->
...
//      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO . '<br /><br /><strong>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</strong>');
    ...
    default:
      if (isset($oInfo) && is_object($oInfo)) {
		...
		/** Econt Shipping Module BOF **/
		if(MODULE_SHIPPING_ECONT_STATUS == 'True') {
			$statuscheck = $db->Execute("SELECT * FROM ".MODULE_SHIPPING_ECONT_DB_LOADINGS." WHERE order_id = ".(int)$oInfo->orders_id."");
			if($statuscheck->RecordCount()) {
				//$statuscheck = tep_db_fetch_array($statuscheck_query);
				if(($statuscheck->fields['is_imported'] == '1') && ($statuscheck->fields['receiver_time'] == "0000-00-00 00:00:00")) {
					$statuscheck_icon = 'forward.png';
				...
				$contents[] = array('text' => '<br /><b>' . MODULE_SHIPPING_ECONT_ICON_LOADING_INFO_TEXT . '</b><br/>' .'<div id="loadingurl-form">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/'.$statuscheck_icon, MODULE_SHIPPING_ECONT_ICON_LOADING_INFO_TEXT).'</div><br/><b>' . MODULE_SHIPPING_ECONT_ICON_LOADING_UPDATE_TEXT . '</b><br/>' .'<div class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button"><span class="ui-button-text">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/shop_orders.png', MODULE_SHIPPING_ECONT_ICON_LOADING_UPDATE_TEXT, "", "", 'onclick="location.href=\''.zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=update_loadings').'\'"').'</span></div>');
			} else {
				$contents[] = array('text' => '<br /><b>' . MODULE_SHIPPING_ECONT_ICON_REQUEST_COURIER_TEXT . '</b><br/>' .'<div id="requestcourier-form">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/shop_users.png', MODULE_SHIPPING_ECONT_ICON_REQUEST_COURIER_TEXT).'</div>'.zen_draw_hidden_field('econtorderid', $oInfo->orders_id, 'id="econtorderid"').'<br/><b>' . MODULE_SHIPPING_ECONT_ICON_LOADING_UPDATE_TEXT . '</b><br/><div class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" role="button"><span class="ui-button-text">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/shop_orders.png', MODULE_SHIPPING_ECONT_ICON_LOADING_UPDATE_TEXT, "", "", 'onclick="location.href=\''.zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=update_loadings').'\'"').'</span></div>');
			}
		}
		/** Econt Shipping Module EOF **/
