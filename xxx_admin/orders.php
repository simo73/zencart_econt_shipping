...
if(MODULE_SHIPPING_ECONT_STATUS == 'True') {
    ...
    include('../' . DIR_WS_MODULES . 'shipping/econt/admin/orders_econt_apps_top.php');
} 
?>
...
<?php if(MODULE_SHIPPING_ECONT_STATUS == 'True') { ?>
    <?php include_once('../' . DIR_WS_MODULES . 'shipping/econt/admin/orders_popup_shop_config.php'); ?>
<?php } ?>
<!-- Econt Shipping Module EOF //-->
<!-- header //-->
      ...
      <tr>
		...
		if(MODULE_SHIPPING_ECONT_STATUS == 'True') {				
			...
			if($statuscheck->RecordCount()) {
				...
				$econt_button = '<div id="loadingurl-form">'. zen_image(DIR_WS_CATALOG_IMAGES . 'infobox/next.png', MODULE_SHIPPING_ORDERS_LOADINGINFO_LEGEND_TEXT).'</div>&nbsp;';
			} else {
...
<?php if(MODULE_SHIPPING_ECONT_STATUS == 'True') { ?>
	<?php
	$statuscheck = $db->Execute("SELECT * FROM ".MODULE_SHIPPING_ECONT_DB_LOADINGS." WHERE order_id = ".(int)$orders->fields['orders_id']."");
	if(!$statuscheck->RecordCount()) {
	...
	?>
	<td class="dataTableContent"><?php echo $econt_courier_button; ?><?php echo '<a href="' . zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders->fields['orders_id'] . '&action=edit', 'NONSSL') . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $orders->fields['customers_name']; ?></td>
<?php } ?>
<!-- Econt Shipping Module BOF //-->
           <?php if(MODULE_SHIPPING_ECONT_STATUS == 'False') { ?>
                <td class="dataTableContent"><?php echo '<a href="' . zen_href_link(FILENAME_CUSTOMERS, 'cID=' . $orders->fields['customers_id'], 'NONSSL') . '">' . zen_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW . ' ' . TABLE_HEADING_CUSTOMERS) . '</a>&nbsp;' . $orders->fields['customers_name'] . ($orders->fields['customers_company'] != '' ? '<br />' . $orders->fields['customers_company'] : ''); ?></td><?php } ?>
