...
if ($conn)
{
	...
	//if ($lang_dop == 'bg') { 
     mysql_query("SET NAMES utf8");
     $fetch = mysql_query("SELECT * FROM econt_cities_streets_table where id_city like " . $test1 . " AND (street_name Like '%" . $_GET['term'] . "%' OR street_name_en Like '%" . $_GET['term'] . "%')"); 
//	}
//	else {
//	mysql_query("SET NAMES utf8");
//     $fetch = mysql_query("SELECT * FROM econt_cities_streets_table where street_name_en like '%" . $_GET['term'] . "%' and id_city like " . $test1 . ""); 
	...
	while ($row = mysql_fetch_array($fetch, MYSQL_ASSOC)) {
		$row_array['label'] = $row['street_name'];
		$row_array['value'] = $row['street_name'];
